...
export const JAIL_CONFIG: JailConfig = {
  templateDataset:
    process.env.NANOCLAW_TEMPLATE_DATASET || 'zroot/nanoclaw/jails/template',
  ...
  jailsPath:
    process.env.NANOCLAW_JAILS_PATH || path.join(NANOCLAW_ROOT, 'jails'),
  workspacesPath:
    process.env.NANOCLAW_WORKSPACES_PATH ||
    path.join(NANOCLAW_ROOT, 'workspaces'),
...
export const MAX_EPAIRS = Math.min(
  255,
  Math.max(1, parseInt(process.env.NANOCLAW_MAX_EPAIRS || '200', 10) || 200),
);
...
export const MAX_CONCURRENT_JAILS = Math.min(
  100,
