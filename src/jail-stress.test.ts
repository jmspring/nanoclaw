...
describe.skipIf(!isFreeBSD || !isRoot)(
  'Jail Stress Test — Rapid Create/Destroy',
  () => {
    ...
    afterAll(() => {
      ...
      if (timings.length > 0) {
        ...
        for (const t of timings) {
          console.log(
            `  ${t.groupId}: create=${t.createMs}ms destroy=${t.destroyMs}ms`,
          );
        }
        console.log('------------------------------\n');
      }
    ...
    it(
      ...
      async () => {
        ...
        for (const groupId of groupIds) {
          ...
          expect(datasetExists(`${JAIL_CONFIG.jailsDataset}/${jailName}`)).toBe(
            true,
          );
    ...
    it(
      ...
      () => {
        ...
        const newEpairs = currentEpairs.filter(
          (e) => !baselineEpairs.includes(e),
        );
    ...
    it(
      ...
      () => {
        ...
        const orphanJails = currentJails.filter(
          (j) =>
            j.includes(STRESS_GROUP_PREFIX) &&
            !baselineRunningJails.includes(j),
        );
